...
export default function MindMeld({ user, partnerProfile }: { user?: any, partnerProfile?: any }) {
  ...
  const partnerColor = partnerProfile?.aura_color || "#EC4899"; // Default Pink

  useEffect(() => { userRef.current = user; }, [user]);

  useEffect(() => {
    // ... (Socket logic remains identical to previous version) ...
    // ... Keep handleNewQuestion, handleReveal, etc. exactly as they were ...
    ...
    const handlePartnerTyping = (isTyping: boolean) => {
      setPartnerStatus((prev) => {
          ...
          return isTyping ? `${partnerName} is typing... 💬` : "Waiting...";
      });
    };

    const handlePartnerSubmitted = () => {
      setPartnerStatus(`${partnerName} is Ready! ✅`);
    };

    const handleReveal = async (data: { answer: string }) => {
      setPartnerAnswer(data.answer);
      setGamePhase("syncing");
      ...
      if (currentQ && currentA && data.answer) {
          ...
          await supabase.from('history').insert({
            ...
            content: { question: currentQ, answerA: currentA, answerB: data.answer }
          });
      }
    };
  ...
  }, [partnerName, roomId]); // Added partnerName dependency
  ...
  const generateQuestion = (vibe: string) => {
    ...
    setTimeout(() => { if (loading) setLoading(false); isButtonLocked.current = false; }, 5000);
  };
  ...
  const handleSubmit = () => {
    ...
    setPartnerStatus((prev) => prev.includes(partnerName) ? prev : `Waiting for ${partnerName}...`);
  };
  ...
  // PSYCHIC REVEAL ANIMATION (Using Aura Colors)
    ...
    return (
      <div className="flex flex-col items-center justify-center w-full h-[60vh] relative" onClick={triggerReveal}>
         <div className="absolute inset-0 flex items-center justify-center">
            {/* Left Orb (You - Default Purple/User Color) */}
            <motion.div 
            ...
            />
            {/* Right Orb (Partner - Dynamic Color) */}
            <motion.div 
              initial={{ x: 150, opacity: 0, scale: 0.5 }}
              animate={{ x: 20, opacity: 1, scale: 1 }}
              transition={{ duration: 1.5, type: "spring", delay: 0.2 }}
              className="w-24 h-24 rounded-full blur-xl absolute mix-blend-screen"
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-md p-6 space-y-6 text-white animate-in slide-in-from-bottom-4">
      ...
      <div className="flex flex-col w-full space-y-4">
        ...
        {/* PARTNER */}
        <div className="space-y-1">
          <label className="text-xs text-zinc-500 ml-2 font-bold tracking-wider uppercase">{partnerName}</label>
          <motion.div 
            layout
            className={`w-full min-h-[58px] bg-zinc-900/50 border border-zinc-700 rounded-xl px-4 py-4 flex items-center justify-between ${gamePhase === "revealed" ? "text-white" : "text-zinc-500 italic"}`}
            style={gamePhase === "revealed" ? { 
                background: `linear-gradient(to right, ${partnerColor}10, ${partnerColor}30)`,
                borderColor: `${partnerColor}50` 
            } : {}}
          >
            {gamePhase === "revealed" ? (
                ...
                <span className="flex items-center gap-2 text-sm" style={{ color: partnerStatus.includes("typing") ? partnerColor : undefined }}>
                    {partnerStatus}
                </span>
            )}
          </motion.div>
      ...
      {gamePhase === "waiting" && (
         <div className="text-zinc-500 text-sm animate-pulse flex items-center gap-2">
            <Loader2 className="animate-spin" size={16} /> Waiting for {partnerName}...
         </div>
      )}
